...
using System.Data.SqlClient;
    ...
    [TableMap("OpenWay", "FItemID")]
    public class OpenWay : EntryObject, IItemInfo
    {
        ...
        public void Save()
        {
            ...
            {
                //子类调用基类的属性
                FItemID = SqlCom.GetMaxNum("OpenWay");
        ...
        public OpenWay[] LoadList(string cond)
        {
            string strSQL = string.Format("select * from OpenWay where {0}", cond);
            using (DbSession db = Db.Get())
            {
                db.OpenConnection();
                return db.QueryList<OpenWay>(strSQL);
            }
        }

        public OpenWay LoadEntity(int FItemID)
        {
            //参数里面设置模糊查询
            string strSql = "";
            SqlParameter pars = null;
            strSql = "select * from OpenWay where FItemID =@FItemID and FDeleted=0 order by FNumber";
            pars = new SqlParameter("@FItemID", FItemID);
            using (DbSession db = Db.Get())
            {
                db.OpenConnection();
                return db.QueryObject<OpenWay>(strSql, pars);
            }
        }
    }
    ...
    public interface IOpenWay
    {
        OpenWay LoadEntity(int FItemID);

        OpenWay[] LoadList(string cond);

        void Save(OpenWay instance);
